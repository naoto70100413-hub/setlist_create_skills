...
1. **掛け持ちチーム間隔（最重要）**: 掛け持ちメンバーのいるチーム同士は最低 **1時間** の間隔
   - 出演順を決める段階で、間にブロック間休憩（最低10分）が挟まる配置は、その分の時間も間隔に加算して評価する。休憩を挟む配置を活用することで、より少ないチーム数の間隔で1時間制約を満たせるようにする
   - 物理的に満たせない場合: 複数ペアの間隔をできるだけ均等化（例: A-B 10分・B-C 60分より A-B 35分・B-C 35分を優先）し ⚠ 警告を出力
2. **ブロック均等割り**: 全チームを 4〜5 ブロックに均等分割（端数は先頭ブロックへ）
3. **ブロック間休憩**: 最低 **10分** 確保しつつ、次ブロックは **15分刻み（00/15/30/45分）** のキリの良い時刻に開始（休憩が15分未満でもよい）
4. **チーム間インターバル**: チーム間は **30秒** の入退場インターバル
5. **希望時間帯（ソフト制約）**: 前半・後半の希望を可能な範囲で反映

いずれかのルール（掛け持ち間隔・希望時間帯など）を守れなかったチームは、出力Excelの**備考欄**にその理由を記載し、行を黄色背景で強調する。
...
- 何らかのルール（掛け持ち間隔・希望時間帯など）を守れなかったチームは**黄色背景**で強調し、備考欄に理由を記載

列構成:
| 出演順 | ブロック内番号 | 出演開始時間 | チーム名 | 曲名 | アーティスト名 | 出演時間 | 入りはけ時間 | 掛け持ちチーム... | 備考 |
|---|---|---|---|---|---|---|---|---|---|
| 1 | 1-1 | 13:00:00 | TEAM_A | ... | ... | 3:45 | 0:30 | TEAM_C | 掛け持ち間隔不足: TEAM_Cとの間隔が0:30:50（必要:1時間以上） |
| 2 | 1-2 | 13:04:15 | TEAM_B | ... | ... | 4:12 | 0:30 | | |
| 出演順 | ブロック内番号 | 出演開始時間 | チーム名 | 曲名 | アーティスト名 | 出演時間 | 入りはけ時間 | 掛け持ちチーム... | 備考 |
| 3 | 2-1 | 13:30:00 | TEAM_C | ... | ... | 3:20 | 0:30 | TEAM_A | 掛け持ち間隔不足: TEAM_Aとの間隔が0:30:50（必要:1時間以上） |

- **備考**: 掛け持ち間隔不足・希望時間帯未達などルール違反があるチームにのみ理由を記載（複数ある場合は`; `区切り）。何も違反がなければ空欄。
